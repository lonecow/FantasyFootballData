...
(function() {
    ...
    try {
            ...
            try {
                ...
                for (var adIndex in EBG.ads) {
                    if (EBG.ads.hasOwnProperty(adIndex)) {
                        ...
                        cfg = ad._adConfig;
                        if ((comAdid == cfg.adId) || (comPlid == cfg.placementId)) {
                            //found the target ad
                            mmSetupComScore(ad, cfg.placeHolderId, gEbQueries_ComScore.scriptURL);
                            foundViaEBG = true;
                            //break;                        
                        }
                    }
     ...
     function mmSetupComScore(obj, id, url) {
		if(!obj.ebComscoreDummy){
			if(window.EBG && !(document.getElementById(id) || EBG.adaptor.getDisplayWin().document.getElementById(id))){
				return false;
			} 
    ...
    function mmSetupComScore2(obj, id, scriptURL) {
        ...
        if (window.EBG) {
            ...
            try {
				...
				if(divTemp.tagName.toLowerCase() == "object"){
					div = divTemp;
				}
				else if(divTemp.tagName.toLowerCase() == "embed"){
					div = divTemp;
				}
                else if (divTemp && divTemp.childNodes.length > 0) {
                    ...
                    for (var i = 0; i < divTemp.childNodes.length; i++) {
                        ...
                        if (tagName) {
                            if (tagName == "img") {
                                div = divTemp;
                                //we have a banner asset present now
                                break;
        ...
        if (!div && !window.EBG) {
			...
			try {
				if (obj.displayUnit.ad.strRichFlash && obj.displayUnit.ad.strRichFlash!=''){
					defaultSet = id.replace('ebBannerDiv_', 'ebBannerFlash_');
					defaultDiv = doc.getElementById(defaultSet);
					if (defaultDiv != null) {
                        return;
                    }
        ...
        if (typeof (div) != 'undefined' && div) {
            ...
            if (defaultSet && typeof (defaultSet) != 'undefined') {
                if (id.indexOf("Rich") > -1) {
                    var defaultDiv = doc.getElementById(defaultSet);
                    if (defaultDiv != null) {
                        return;
                    }
                }
            }
			window.clearInterval(obj.ebComscoreDummy);
